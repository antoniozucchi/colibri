...
uses
  ...
  Vcl.Samples.Spin,UITYPES,ClipBrd, untDBGridFilter;
...
type
  TFrmGerenciarEmbarcacoes = class(TForm)
    ...
    actFiltroInserirRoteamento: TAction;
    actGridASCRotemento: TAction;
    actGridDESCRoteamento: TAction;
    actLimparFiltrosRoteamento: TAction;
    actFiltrosTabelaRoteamento: TAction;
    actProcuraFiltrosTabelaRoteamento: TAction;
    ...
    procedure actExcelProcuraExecutantesExecute(Sender: TObject);
    ...
    procedure DBGridRoteamentoTitleClick(Column: TColumn);
    procedure actSubstituirPorExecute(Sender: TObject);
    procedure actLimparFiltrosRoteamentoExecute(Sender: TObject);
    procedure actProcuraRoteamentoExecute(Sender: TObject);
    procedure actProcuraFiltrosTabelaRoteamentoExecute(Sender: TObject);
    procedure actFiltrosTabelaRoteamentoExecute(Sender: TObject);
  ...
  private
    BooleanIndividual: Boolean;
    MatrizExecSAP: array of array of String;
    { Private declarations }

    procedure WMMDIACTIVATE(var msg: TWMMDIACTIVATE);message WM_MDIACTIVATE;
    procedure editarRoteamento(Tipo: Integer);
    function StatusProgramacaoRT(CodigoSAP,Documento: String): String;
...
implementation
  uses untDataModule,UntPrincipal,untFrmPreview;
{$R *.dfm}
...
procedure TFrmGerenciarEmbarcacoes.actExcelProcuraExecutantesExecute(
  Sender: TObject);
begin
  if btnProgramados.Down then
    FrmPrincipal.GerarExcel(DBGridProcuraProgramados,'Excecutantes Programados')
  else
    FrmPrincipal.GerarExcel(DBGridProcuraGeral,'Excecutantes Programados')
end;

procedure TFrmGerenciarEmbarcacoes.actExcelDestinosExecute(Sender: TObject);
begin
  FrmPrincipal.ExcelStringGrid(StrGridResumo,'Destinos',
  '','',1);
end;

procedure TFrmGerenciarEmbarcacoes.actExcelExecute(Sender: TObject);
begin
  FrmPrincipal.GerarExcel(DBGridRotaExecutantes,'Roteiro Executantes');
end;
...
begin
  ...
  edtTitulo1.Text,edtTitulo2.Text,3);
end;
...
begin
  ...
  actSalvarRota.Execute;
  FrmPrincipal.AutoFitGrade(StrGridRota);
...
procedure TFrmGerenciarEmbarcacoes.actExcluirVinculoExecuntanteSelecionadoExecute(
  Sender: TObject);
  var
...
begin
  try
    ...
    'Excluindo executantes selecionados...');
    FrmDataModule.ADOQueryTM_RotaExecutantes.First;
    while not FrmDataModule.ADOQueryTM_RotaExecutantes.Eof do
    begin
      if FrmDataModule.DataSourceTM_RotaExecutantes.
      DataSet.FieldByName('booleanSelecao').AsBoolean then
      begin
        //Excluir vincluo
    ...
    FrmPrincipal.AutoFitStatusBar(StatusBarRotaExecutantes);
...
procedure TFrmGerenciarEmbarcacoes.actProcuraFiltrosTabelaRoteamentoExecute(
  Sender: TObject);
begin
  frmPrincipal.CarregaFiltrosProcura(ColunasLayoutRoteamento);
  ...
  try
    ...
    begin
      try
        ...
        if i > 1 then
        begin
          Distancia:= RoundTo(FrmPrincipal.GetDistanceBetween(Lat1,Long1,Lat2,Long2),-3);
          Long1:= Long2;
          Lat1:= Lat2;
...
begin
  ...
  edtTitulo2.Text:= 'DATA: '+ DataProcura;
  FrmPrincipal.ProgramacaoDuplicados(RLImpressao,DataProcura);
end;
...
procedure TFrmGerenciarEmbarcacoes.actLimparFiltrosRoteamentoExecute(
  Sender: TObject);
begin
  FrmPrincipal.LimparColunasFiltro(DBGridRoteamento,ColunasLayoutRoteamento);
  actProcuraRoteamento.Execute;
end;

procedure TFrmGerenciarEmbarcacoes.actListaRTEmbarqueExecute(Sender: TObject);
  var
    DataProcura,SQLBase,CPF,CodigoSAP,PRG_TM,Empresa,
    txtTipoEtapaServico,NomeExecutante,Mensagem,Origem,SQLString: String;
    numLinhas,i: Integer;
    ListaNaoDefinido: TStringList;
begin
  ...
  begin
    ...
    MatrizExecSAP:= nil;
    SetLength(MatrizExecSAP, 5);
    for i := 0 to High(MatrizExecSAP) do
      SetLength(MatrizExecSAP[i],
      FrmDataModule.ADOQueryTemporarioDBColibri.RecordCount);
    //==============================================
    ...
    begin
      ...
      RLImpressao.Cells[5,numLinhas]:= PRG_TM+FrmDataModule.
      DataSourceTemporarioDBColibri.DataSet.FieldByName('Origem').AsString+'->'+
      FrmDataModule.DataSourceTemporarioDBColibri.DataSet.FieldByName('txtDestino').AsString;
      //===========================================================================
      MatrizExecSAP[0,i]:= FrmDataModule.DataSourceTemporarioDBColibri.DataSet.
      FieldByName('CodigoSAP').AsString;
      MatrizExecSAP[1,i]:= FrmDataModule.DataSourceTemporarioDBColibri.DataSet.
      FieldByName('Documento').AsString;
      MatrizExecSAP[2,i]:= FrmDataModule.DataSourceTemporarioDBColibri.DataSet.
      FieldByName('InseridoProgramacaoTransporte').AsString;
      MatrizExecSAP[3,i]:= FrmDataModule.DataSourceTemporarioDBColibri.DataSet.
      FieldByName('Origem').AsString;
      MatrizExecSAP[4,i]:= FrmDataModule.DataSourceTemporarioDBColibri.DataSet.
      FieldByName('txtDestino').AsString;
      FrmDataModule.ADOQueryTemporarioDBColibri.Next;
      i:= i+1;
      FrmPrincipal.ProgressBarIncremento(1);
    ...
    //==============================================
    ListaNaoDefinido:= TStringList.Create;
    ListaNaoDefinido.Clear;
    FrmDataModule.ADOQueryExecutantesSAP.Active:= false;
    FrmDataModule.ADOQueryExecutantesSAP.Parameters.Items[0].Value:= DataProcura;
    FrmDataModule.ADOQueryExecutantesSAP.Active:= true;
    FrmDataModule.ADOQueryExecutantesSAP.First;
    FrmPrincipal.ProgressBarIncializa(FrmDataModule.ADOQueryExecutantesSAP.RecordCount,
    'Carregando Não Programados...');
    while not FrmDataModule.ADOQueryExecutantesSAP.Eof do
    begin
      Empresa:= FrmDataModule.DataSourceExecutantesSAP.DataSet.
      FieldByName('txtEmpresa').AsString;
      if (Empresa = 'NÃO DEFINIDO') then
      begin
        ListaNaoDefinido.Add(FrmPrincipal.TextoMaiusculo((FrmDataModule.
        DataSourceExecutantesSAP.DataSet.FieldByName('NomeExecutante').AsString)));
      end
      else if (StatusProgramacaoRT(FrmDataModule.DataSourceExecutantesSAP.DataSet.
      FieldByName('CodigoSAP').AsString,FrmDataModule.DataSourceExecutantesSAP.
      DataSet.FieldByName('Documento').AsString) = 'NP')AND(Empresa <> 'NÃO DEFINIDO') then
      begin
        NomeExecutante:= FrmPrincipal.TextoMaiusculo((FrmDataModule.DataSourceExecutantesSAP.DataSet.
        FieldByName('NomeExecutante').AsString));
        txtTipoEtapaServico:= FrmDataModule.DataSourceExecutantesSAP.DataSet.
        FieldByName('txtTipoEtapaServico').AsString;
        CodigoSAP:= FrmDataModule.DataSourceExecutantesSAP.DataSet.
        FieldByName('CodigoSAP').AsString;
        CPF:= FrmDataModule.DataSourceExecutantesSAP.DataSet.
        FieldByName('Documento').AsString;
        if CPF = '' then
          CPF := '000.000.000-00';
        //iserirNP(Empresa,NomeExecutante,txtTipoEtapaServico,CodigoSAP,Documento);
        numLinhas:= RLImpressao.RowCount;
        RLImpressao.RowCount:= numLinhas+1;
        //Preencher valores
        RLImpressao.Cells[0,numLinhas]:= Empresa;
        RLImpressao.Cells[1,numLinhas]:= NomeExecutante;
        RLImpressao.Cells[2,numLinhas]:= txtTipoEtapaServico;
        RLImpressao.Cells[3,numLinhas]:= CodigoSAP;
        RLImpressao.Cells[4,numLinhas]:= CPF;
        RLImpressao.Cells[5,numLinhas]:= 'NP';
      end;
      FrmDataModule.ADOQueryExecutantesSAP.Next;
      FrmPrincipal.ProgressBarIncremento(1);
    end;
    ...
    FrmPrincipal.AutoFitGrade(RLImpressao);
    FrmPrincipal.ProgressBarAtualizar;
    if ListaNaoDefinido.Count > 0 then
    begin
      Mensagem:= '';
      for I := 0 to ListaNaoDefinido.Count -1 do
        Mensagem:= Mensagem+#13+IntToStr(i+1)+'. '+ListaNaoDefinido[i];

      ShowMessage('NÃO DEFINIDOS:'+#13+Mensagem);
    end;
  end;
end;
...
begin
  if FrmPrincipal.PanelAjuda1.Visible then
    FrmPrincipal.PanelAjuda1.Visible:= false;
...
begin
  FrmPrincipal.PainelDuplicados(DateToStr(DateTimePickerProgramacao.Date));
end;
...
procedure TFrmGerenciarEmbarcacoes.actRT_TMIBExecute(Sender: TObject);
  var
    DataProcura,SQLBase,SQLOrigem: String;
    numLinhas,i: Integer;
begin
  RadioGroupImpressao.ItemIndex:= 1;
  //Procurar todos os Executantes Programados para a DataProcura
  ...
  'ORDER BY NomeExecutante,txtDestino,txtTipoEtapaServico;';
  FrmDataModule.ADOQueryTemporarioDBColibri.Close;
  FrmDataModule.ADOQueryTemporarioDBColibri.SQL.Clear;
  FrmDataModule.ADOQueryTemporarioDBColibri.SQL.Add(SQLBase);
  FrmDataModule.ADOQueryTemporarioDBColibri.Open;
  //Matrix, (0)CodigoSAP, (1)InseridoProgramacaoTransporte, (2)StatusProgramacao
  MatrizExecSAP:= nil;
  SetLength(MatrizExecSAP, 5);
  for i := 0 to High(MatrizExecSAP) do
    SetLength(MatrizExecSAP[i],
    FrmDataModule.ADOQueryTemporarioDBColibri.RecordCount);
  //Carregar matrix de executantes programados
  i:= 0;
  FrmDataModule.ADOQueryTemporarioDBColibri.First;
  while not FrmDataModule.ADOQueryTemporarioDBColibri.Eof do
  begin
    MatrizExecSAP[0,i]:= FrmDataModule.DataSourceTemporarioDBColibri.
    DataSet.FieldByName('CodigoSAP').AsString;
    MatrizExecSAP[1,i]:= FrmDataModule.DataSourceTemporarioDBColibri.
    DataSet.FieldByName('Documento').AsString;
    MatrizExecSAP[2,i]:= FrmDataModule.DataSourceTemporarioDBColibri.
    DataSet.FieldByName('InseridoProgramacaoTransporte').AsString;
    MatrizExecSAP[3,i]:= FrmDataModule.DataSourceTemporarioDBColibri.
    DataSet.FieldByName('Origem').AsString;
    MatrizExecSAP[4,i]:= FrmDataModule.DataSourceTemporarioDBColibri.
  ...
  RLImpressao.Cells[1,0]:= 'Nome do Executante';
  RLImpressao.Cells[2,0]:= 'Tipo de Etapa de Serviço';
  RLImpressao.Cells[3,0]:= 'RT';
  RLImpressao.Cells[4,0]:= 'Tipo';
  RLImpressao.Cells[5,0]:= 'Status';
  FrmPrincipal.ProgressBarIncializa(FrmDataModule.ADOQueryExecutantesSAP.RecordCount,
  'Carregando todas as RT''s de embarque e verificar executantes programados...');
  while not FrmDataModule.ADOQueryExecutantesSAP.Eof do
  begin
    numLinhas:= RLImpressao.RowCount;
    RLImpressao.RowCount:= numLinhas+1;
    //Preencher valores
    RLImpressao.Cells[0,numLinhas]:= (FrmDataModule.
    DataSourceExecutantesSAP.DataSet.FieldByName('txtEmpresa').AsString);
    RLImpressao.Cells[1,numLinhas]:= FrmPrincipal.TextoMaiusculo((FrmDataModule.
    DataSourceExecutantesSAP.DataSet.FieldByName('NomeExecutante').AsString));
    RLImpressao.Cells[2,numLinhas]:= (FrmDataModule.
    DataSourceExecutantesSAP.DataSet.FieldByName('txtTipoEtapaServico').AsString);
    RLImpressao.Cells[3,numLinhas]:= (FrmDataModule.
    DataSourceExecutantesSAP.DataSet.FieldByName('Nota').AsString);
    RLImpressao.Cells[4,numLinhas]:= (FrmDataModule.
    DataSourceExecutantesSAP.DataSet.FieldByName('TipoNota').AsString);
    RLImpressao.Cells[5,numLinhas]:= (StatusProgramacaoRT(FrmDataModule.
    DataSourceExecutantesSAP.DataSet.FieldByName('CodigoSAP').AsString,
    FrmDataModule.DataSourceExecutantesSAP.DataSet.FieldByName('Documento').AsString));
    FrmDataModule.ADOQueryExecutantesSAP.Next;
    FrmPrincipal.ProgressBarIncremento(1);
  end;
  StatusBarImpressao.Panels[0].Text:= 'N° registros: '+
...
procedure TFrmGerenciarEmbarcacoes.actSalvarRotaExecute(Sender: TObject);
  var
    RotaSequencia: String;
    i: Integer;
begin
  RotaSequencia:= '';
  for I := 1 to StrGridRota.RowCount-1 do
  begin
    if RotaSequencia <> '' then
      RotaSequencia:= RotaSequencia+';'+StrGridRota.Cells[1,i]
    else
      RotaSequencia:= StrGridRota.Cells[1,i];
  end;
  FrmDataModule.ADOQueryRoteamento.Edit;
...
procedure TFrmGerenciarEmbarcacoes.actSubstituirPorExecute(Sender: TObject);
begin
  FrmPrincipal.SubstituirPor(DBGridRoteamento,FrmDataModule.ADOQueryRoteamento,
  FrmDataModule.DataSourceRoteamento);
end;
...
begin
  ...
  CodigoRota:= FrmDataModule.DataSourceRoteamento.DataSet.
  FieldByName('idRoteamento').AsInteger;
  FrmPrincipal.ProgressBarIncializa(FrmDataModule.ADOQueryConsultaExecutantes_DataProgramacao.RecordCount,
  'Vinculando executantes a rota selecionada...');
  FrmDataModule.DataSourceConsultaExecutantes_DataProgramacao.Enabled:= false;
  FrmDataModule.ADOQueryConsultaExecutantes_DataProgramacao.First;
  while not FrmDataModule.ADOQueryConsultaExecutantes_DataProgramacao.Eof do
  begin
    if FrmDataModule.DataSourceConsultaExecutantes_DataProgramacao.
    DataSet.FieldByName('booleanSelecao').AsBoolean then
    begin
      CodigoProgramacaoExecutante:=
      FrmDataModule.DataSourceConsultaExecutantes_DataProgramacao.
      DataSet.FieldByName('idProgramacaoExecutante').AsInteger;
      //Procurar Executante e marcar como inserido na programação TM
      FrmDataModule.ADOQueryConsultaProgramacaoExecutante_ID.Active:= false;
      FrmDataModule.ADOQueryConsultaProgramacaoExecutante_ID.Parameters.Items[0].Value:=
      CodigoProgramacaoExecutante;
      FrmDataModule.ADOQueryConsultaProgramacaoExecutante_ID.Active:= true;
      ...
      FrmDataModule.ADOQueryConsultaProgramacaoExecutante_ID.Post;
      //================================================
      FrmDataModule.ADOQueryAux_Rota_Distribuicao.Insert;
      FrmDataModule.DataSourceAux_Rota_Distribuicao.DataSet.
      FieldByName('CodigoProgramacaoExecutante').AsInteger:= CodigoProgramacaoExecutante;
      FrmDataModule.DataSourceAux_Rota_Distribuicao.DataSet.
      FieldByName('CodigoRota').AsInteger:= CodigoRota;
      FrmDataModule.ADOQueryAux_Rota_Distribuicao.Post;
    end;
    FrmDataModule.ADOQueryConsultaExecutantes_DataProgramacao.Next;
    FrmPrincipal.ProgressBarIncremento(1);
  end;
...
function TFrmGerenciarEmbarcacoes.StatusProgramacaoRT(CodigoSAP,
...
begin
  ...
  if (codigoSAP = '')OR(codigoSAP = '0') then
    codigoSAP:= '999999999';
  for i := 0 to High(MatrizExecSAP[0]) do
  begin
    if ((MatrizExecSAP[0,i] = codigoSAP)OR(MatrizExecSAP[1,i] = Documento)) then
    begin
      if UpperCase(MatrizExecSAP[2,i]) = 'TRUE' then
        Result:= 'TM:'+MatrizExecSAP[3,i]+'->'+MatrizExecSAP[4,i]
      else
        Result:= 'PRG:'+MatrizExecSAP[3,i]+'->'+MatrizExecSAP[4,i];
...
begin
  ...
  FrmPrincipal.AutoFitGrade(StrGridResumo);
end;

procedure TFrmGerenciarEmbarcacoes.actFiltrosTabelaRoteamentoExecute(
  Sender: TObject);
begin
  FrmPrincipal.btnProcurarFiltrosTabela.Action:= actProcuraFiltrosTabelaRoteamento;
  FrmPrincipal.FiltrosTabela(DBGridRoteamento,ColunasLayoutRoteamento);
end;

procedure TFrmGerenciarEmbarcacoes.actFinalGridImpressaoExecute(
  Sender: TObject);
begin
  FrmPrincipal.AutoFitStatusBar(StatusBarImpressao);
...
procedure TFrmGerenciarEmbarcacoes.DBGridRoteamentoTitleClick(Column: TColumn);
begin
  FrmPrincipal.configurarFiltro(1,Column.FieldName,IntToStr(Column.Index),
  Column.ReadOnly,actFiltroInserirRoteamento,actGridASCRotemento,
  actGridDESCRoteamento,actSubstituirPor);
  //======================================================
  FrmPrincipal.titleGrid(ColunasLayoutRoteamento,'Colibri',
  FrmDataModule.ADOQueryRoteamento.SQL.Text);
end;
...
begin
  ...
  FrmPrincipal.ProgressBarIncremento(1);
  FrmPrincipal.SetUpColunasLayout(DBGridProcuraProgramados, ColunasLayoutProgramados);
  FrmPrincipal.SetUpColunasLayout(DBGridProcuraGeral, ColunasLayoutProcuraGeral);
  FrmPrincipal.SetUpColunasLayout(DBGridRotaExecutantes, ColunasLayoutRotaExecutantes);
  FrmPrincipal.SetUpColunasLayout(DBGridRoteamento, ColunasLayoutRoteamento);
  FrmPrincipal.ProgressBarIncremento(1);
