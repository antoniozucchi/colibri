...
procedure TFrmEmbarcacao.DBGridEmbarcacaoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if (DBGridEmbarcacao.SelectedField.FieldName = 'TipoEmbarcacao')OR
  (DBGridEmbarcacao.SelectedField.FieldName = 'StatusEmbarcacao') then
    Key:= #0;
end;
...
procedure TFrmEmbarcacao.FormDestroy(Sender: TObject);
begin
  FrmPrincipal.MDIChildDestroyed(self.Handle);
end;

procedure TFrmEmbarcacao.WMMDIACTIVATE(var msg: TWMMDIACTIVATE);
var
  active: TWinControl;
  idx: Integer;
begin
